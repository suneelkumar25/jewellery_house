...
module.exports = {
  ...
  jewellery_products_post: (name, sku_code, price, material, purity) => {
    return new Promise((resolve, reject) => {
      const insertQuery = `insert into jewellery_products (name, sku_code, price, material, purity) values (?, ?, ?, ?, ?)`;
      const insertValues = [name, sku_code, price, material, purity];
      connection.execute(insertQuery, insertValues, (insertError, insertResult) => {
        if (insertError) {
          console.log(insertError, "InsertError");
